...
class BufferedRoutesTest {
    ...
    @Test
    void bufferedRoutesNameThrowsForInvalidIndex() {
        BufferedRoutes routes = new BufferedRoutes(STRING_TABLE, ROUTES);
        int size = routes.size();

        assertThrows(IndexOutOfBoundsException.class, () -> routes.name(-1));
        assertThrows(IndexOutOfBoundsException.class, () -> routes.name(size));
    }
    @Test
    void bufferedRoutesVehicleThrowsForInvalidIndex() {
        BufferedRoutes routes = new BufferedRoutes(STRING_TABLE, ROUTES);
        int size = routes.size();

        assertThrows(IndexOutOfBoundsException.class, () -> routes.vehicle(-1));
        assertThrows(IndexOutOfBoundsException.class, () -> routes.vehicle(size));
    }
}